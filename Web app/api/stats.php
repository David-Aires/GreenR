...
function getMoyenneMesureTableDate($box,$from,$to){
    global $conn;
    $query="SELECT avg(TEMPERATURE) as TEMPERATURE,avg(HUMIDITY) as HUMIDITY,avg(CO2) as CO2 FROM AIR_STAT natural join CROSS_CENTER where CROSS_CENTER.DATE_TIME between '$from' and '$to' and SERIAL_NUM='$box';";
    $response = array();
    $result = mysqli_query($conn, $query) or die("Error in Selecting " . mysqli_error($conn));
    while($row = mysqli_fetch_object($result))
    {
...
function getMoyenneMesureTableDateUnique($box,$table,$mesure,$from,$to){
    global $conn;
    if($table=="POSITION"){
        $query="SELECT avg($mesure) as $mesure FROM db_GreenR.POSITION natural join CROSS_CENTER WHERE CROSS_CENTER.DATE_TIME between '$from' and '$to' and SERIAL_NUM='$box'";
    }
    else if($table=="AIR_STAT"){
        $query="SELECT avg($mesure) as $mesure FROM db_GreenR.AIR_STAT natural join CROSS_CENTER WHERE CROSS_CENTER.DATE_TIME between '$from' and '$to' and SERIAL_NUM='$box'";
    }
    else if($table=="AIR_BOX"){
        $query="SELECT avg($mesure) as $mesure FROM db_GreenR.AIR_BOX WHERE CROSS_CENTER.DATE_TIME between '$from' and '$to' and SERIAL_NUM='$box'";
    }
    else if($table=="CROSS_CENTER"){
        $query="SELECT avg($mesure) as $mesure FROM db_GreenR.CROSS_CENTER  WHERE CROSS_CENTER.DATE_TIME between '$from' and '$to' and SERIAL_NUM='$box'";
    }
    else{
        header('Content-Type: application/json');
    }
...
/*===========================================================================================*/
	...
	switch($request_method)
	{
		...
		case 'GET':
            if(!empty($_GET["box"]) AND !empty($_GET["table"]) AND !empty($_GET["mesure"]) AND !empty($_GET["from"]) AND !empty($_GET["to"]) AND !empty($_GET["action"]))
            {
                ...
                getMoyenneMesureTableDateUnique($box,$table,$mesure,$from,$to);

            }
		    else if(!empty($_GET["box"]) AND !empty($_GET["table"]) AND !empty($_GET["mesure"]) AND !empty($_GET["from"]) AND !empty($_GET["to"]))
            {
